...
def compute():
    ...
    parser.add_argument('-m', '--max-card', type=str, metavar='PYLIST', default='1,2,3', dest='cards',
    ...
    if args.algorithms == [] or args.algorithms == [['all']]:
        algorithms  = ['hst-yices', 'hst-cache-yices', 'hst-ci-yices', 'hst-ci-cache-yices', 'hsdag-yices', 
                       'hsdag-cache-yices', 'hsdag-ci-yices', 'hsdag-ci-cache-yices', 'hst-picosat', 
                       'hst-cache-picosat', 'hst-ci-picosat', 'hst-ci-cache-picosat', 'hsdag-picosat', 
                       'hsdag-cache-picosat', 'hsdag-ci-picosat', 'hsdag-ci-cache-picosat']
    else:
    ...
    with open(input_file) as f:
        ...
        for line in f.readlines()[1:]:
            l = line.split()
            Problem(file=l[0], ngates=0, nmutations=len(l[1].split(",")), pid=pid)
            pid += 1
            ...
            try:
                ...
                if args.dry_run:
                    ...
                    if task.pid != last_task.pid:
                        ...
                        sisc_file = os.path.join(THIS_FILE_DIR, '..', 'iscas-data', inputline[0] + ".sisc")
                        inputs  = map(lambda s: int(s.split("=")[1]), inputline[2].split(","))
                        outputs = map(lambda s: int(s.split("=")[1]), inputline[3].split(","))
                    ...
                    python_cmd = [PYTHONPATH, os.path.join(THIS_FILE_DIR, '../../pymbd/benchmark/iscas/compute_sample.py'), task.algorithm, str(max_card), str(prune), str(max_time)]
                    ...
                    Result.append(experiment=experiment, run_id=args.run_id, datetime=start, pid=task.pid, exectime=exectime, 
                                  ...
                                  ncomp=0, tp_time_check=stats.get('tp_time_check',0), 
                                  ...
                                  max_rss=max_rss, nrss_polls=num_rss_polls,ngates=problem.ngates,nmutations=problem.nmutations,
